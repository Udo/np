...
static void constructor (LexState *ls, expdesc *t) {
  ...
  checknext(ls, TK_TBLSTART);
  do {
    lua_assert(cc.v.k == VVOID || cc.tostore > 0);
    if (ls->t.token == ')') break;
    closelistfield(fs, &cc);
    field(ls, &cc);
  } while (testnext(ls, ',') || testnext(ls, ';') || 1);
  check_match(ls, ')', TK_TBLSTART, line);
  lastlistfield(fs, &cc);
...
static int explist (LexState *ls, expdesc *v, int limitedBy) {
  /* explist -> expr { `,' expr } */
  int n = 1;  /* at least one expression */
  expr(ls, v);
  while (testnext(ls, ',') || (limitedBy != 0 && ls->t.token != limitedBy)) {
...
static void forlist (LexState *ls, TString *indexname) {
  ...
  checknext(ls, TK_IN);
  line = ls->linenumber;
  adjust_assign(ls, 3, explist(ls, &e, 0), &e);
...
static void test_then_block (LexState *ls, int *escapelist, int requireDo) {
...
static void ifstat (LexState *ls, int line) {
  ...
  test_then_block(ls, &escapelist, 1);  /* IF cond THEN block */
